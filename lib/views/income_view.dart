...
class IncomeView extends StatefulWidget {
  ...
  IncomeView({required this.controller});
...
class _IncomeViewState extends State<IncomeView> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<AddAmountModel>>(
        future: widget.controller.fetchAmounts(true),
            ...
            return Column(
              children: [
                ...
                GlassmorphicContainer(
                  ...
                  child: Center(
                    child: Text(
                      'Total Income: ${totalIncome.toStringAsFixed(2)} Ks',
                      style: GoogleFonts.montserrat(
                        fontSize: 16,
                        color: Colors.green,
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return ListTile(
                        ...
                        subtitle: Text(
                          'Amount (Ks): ${incomes[index].amount}',
                          style: GoogleFonts.montserrat(
                            fontSize: 13,
            ...
            IconButton(
                ...
                onPressed: () {
                  Navigator.push(
                      ...
                      MaterialPageRoute(
                          builder: (context) =>
                              IncomeView(controller: widget.controller)));
                }),
            SizedBox(width: 30),
            IconButton(
                icon: Icon(Icons.trending_down_rounded, color: Colors.white),
                onPressed: () {
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) =>
                              ExpenseView(controller: widget.controller)));
                }),
            IconButton(
  ...
  void _handleMenuSelection(
      BuildContext context, String value, AddAmountModel amount) {
    if (value == 'Edit') {
  ...
  void _showEditDialog(BuildContext context, AddAmountModel amount) {
    final TextEditingController _titleController =
        TextEditingController(text: amount.title);
    final TextEditingController _amountController =
        TextEditingController(text: amount.amount.toString());
            ...
            TextButton(
              ...
              onPressed: () {
                final updatedAmount = AddAmountModel(
                  ...
                  amount:
                      double.tryParse(_amountController.text) ?? amount.amount,
                  date: amount.date,
                );

                widget.controller.updateAmount(updatedAmount, true).then((_) {
        ...
        return AlertDialog(
          ...
          actions: [
            ...
            TextButton(
              ...
              onPressed: () {
                widget.controller.deleteAmount(amount, true).then((_) {
                  Navigator.of(context).pop();
                  setState(() {});
                });
              },
